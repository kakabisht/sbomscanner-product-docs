...
{product_name} can be used in air-gapped environments.

To run the scans, {product_name} currently needs the following external
sources:
...
These external sources can be self-hosted in your private infrastructure
...
Once mirrored in your own OCI registry, you can install {product_name} to
point to them:
...
To configure a VEX Hub in {product_name}, create a `VEXHub` resource with
