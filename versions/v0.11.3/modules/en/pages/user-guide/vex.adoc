...
Here's an example of `VEXHub` resource (you can find it under https://github.com/kubewarden/sbomscanner/blob/main/examples/vexhub.yaml[`examples/vexhub.yaml`]:
...
Then, run the scan applying a https://github.com/alegrey91/sbomscanner/blob/main/examples/scanjob.yaml[`ScanJob`] configured to scan the desired registry.

{sboms-name} will automatically detect the presence of `VEXHub` resources and will include them in the scan.
...
Air Gap support for VEX Hub is described xref:user-guide/airgap-support.adoc#_self-hosting-vex-hub[here].
