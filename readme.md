# SBOM Scanner Product Documentation 
...
```console
git clone https://github.com/rancher/sbomscanner-product-docs
```
...
```console
npx antora --fetch sbomscanner-playbook-local.yml
```
...
As an example, run the command below to use the local `sbomscanner-playbook-local.yml` file.
